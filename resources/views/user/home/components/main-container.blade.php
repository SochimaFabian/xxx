...
<main class="main-container">
    <section class="content-container">
        <div class="content">
            @include('user.home.components.stories')
            <div class="posts">
                        ...
                        <div class="post__header">
                            <div class="post__profile">
                                <a href="{{ route('p.show', $post->user->username) }}" target="_blank" class="post__avatar">
                                    ...
                                    src="{{ asset($post->user->profile->image) }}"
                                    @else
                                    src="{{ asset('home/assets/default-user.png') }}"
                                    @endif alt="User Picture">
                                </a>
                                <a href="{{ route('p.show', $post->user->username) }}" target="_blank" class="post__user"><span>@</span>{{ $post->user->username }}</a>
                            </div>
                        ...
                        <div class="post__footer">
                            ...
                            <div class="post__infos">
                                ...
                                <div class="post__description">
                                    <span>
                                        <a class="post__name--underline" href="{{ route('p.show', $post->user->username) }}" target="_blank"><span>@</span>{{ $post->user->username }}</a>
                                        {{ $post->description }}
                                    </span>
                                </div>
        ...
        <section class="side-menu">
            <div class="side-menu__user-profile">
                <a href="{{ route('p.show', $user->username) }}"  target="" class="side-menu__user-avatar">
                    @if (empty($user->profile->image))
                        <img src="{{ asset('home/assets/default-user.png') }}" alt="User Picture">
                    @else
                        <img src="{{ asset($user->profile->image) }}" alt="User Picture">
                    @endif
                </a>
                <div class="side-menu__user-info">
                    <a href="{{ route('p.show', $user->username) }}" target="_blank" style="text-transform:lowercase">{{ $user->username }}</a>
                    <span style="text-transform: capitalize">{{ ucwords($user->profile->name) }}</span>
                </div>
                <form action="{{ route("logout") }}" method="post">
                    @csrf
                    <button type="submit" class="side-menu__user-button">Log Out</button>
                </form>
            </div>
